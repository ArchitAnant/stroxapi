...
import os
import urllib
from lmdb_datasets import LMDBDataset, HandwritingDataset
...
def get_loaders_eval(dataset, args):
    ...
    elif dataset.startswith('handwriting'):
        num_classes = 1
        resize = 128
        train_transform, valid_transform = _data_transforms_generic(resize)
        train_data = HandwritingDataset(root_dir="data_subset/train", transforms=train_transform)
        valid_data = HandwritingDataset(root_dir="data_subset/test", transforms=valid_transform)
