...
class HandwritingDataset(data.Dataset):
    def __init__(self,root_dir, transforms = None):
        ...
        for style_id in os.listdir(root_dir):
            style_path = os.path.join(root_dir,style_id)
            if os.path.isdir(style_path):
                for fname in os.listdir(style_path):
                    if fname.lower().endswith(('.png','.jpg','.jpeg')):
                        img_path = os.path.join(style_path,fname)
                        self.samples.append((img_path, style_id))

    def __len__(self):
        return len(self.samples)
    
    def __getitem__(self, index):
        img_path, style_id = self.samples[index]
        image = Image.open(img_path).convert('RGB')

        if self.transforms is not None:
            image = self.transforms(image)

        return image, style_id
