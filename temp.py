                ...
                #with open('./utils/splits_words/iam_test.txt', 'r') as f:
                    ...
                    for label in labels:
                        ...
                        if len(matching_lines) >= 5:
                            ...
                            # Fallbacks when <5 matches
                            if len(matching_lines) == 0:
                                # pick any 5 from the dataset
                                five_styles = random.sample(train_data, 5)
                            else:
                                reps = (5 + len(matching_lines) - 1) // len(matching_lines)
                                five_styles = (matching_lines * reps)[:5]
                        ...
                        for im_idx, random_f in enumerate(five_styles):
                            ...
                            # local preprocess identical to training
                            preprocess = transforms.Compose([
                                transforms.ToTensor(),
                                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
                            ])
                            img_tens = preprocess(img_s).to(args.device)
                            st_imgs += [img_tens]
